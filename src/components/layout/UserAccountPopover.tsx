...
import { User, Sun, Moon, Clock, PlaySquare, Bookmark, ThumbsUp, LogOut, Settings, ChevronRight, ShieldCheck, Code2 } from 'lucide-react';
...
export function UserAccountPopover({
  ...
  const { subscribedChannels, watchLaterVideos, likedVideos, playlists } = useUserStore();
    ...
    <>
      ...
      <div className="absolute right-4 top-14 z-50 w-72 bg-white dark:bg-[#212121] text-neutral-900 dark:text-white rounded-2xl shadow-2xl border border-neutral-200 dark:border-white/10 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150 transition-colors">
        ...
        <div className="p-4 border-b border-neutral-200 dark:border-white/10 flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-blue-600 text-white font-bold flex items-center justify-center shrink-0 shadow-md">
            NX
          </div>
          <div className="flex flex-col">
            <span className="font-bold text-sm">Naim Xbz</span>
            <span className="text-xs text-neutral-500 dark:text-white/60">@naimxbz</span>
            <button
        ...
        <div className="p-2">
          <button
            onClick={onClose}
            className="w-full flex items-center gap-3 px-3 py-2 text-xs font-semibold text-red-500 hover:bg-red-500/10 rounded-xl transition-colors"
          >
